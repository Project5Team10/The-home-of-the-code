...
CAN_TxHeaderTypeDef	Tx2Header;      //发送
CAN_RxHeaderTypeDef	Rx2Header;      //接收
...
{	
	...
	u8 message[8];
	  ...
	  TxHeader.RTR=CAN_RTR_DATA;  //数据帧֡
    TxHeader.DLC=len;                
    for(i=0;i<len;i++)
    {
		message[i]=msg[i];
...
{	
	...
	u32 TxMailbox;
	u8 message[8];
	Tx2Header.StdId=0X12;        //标准标识符
	    Tx2Header.ExtId=0x12;        //扩展标识符(29位)
	    Tx2Header.IDE=CAN_ID_STD;    //使用标准帧
	    Tx2Header.RTR=CAN_RTR_DATA;  //数据帧֡
    Tx2Header.DLC=len;                
    for(i=0;i<len;i++)
    {
		message[i]=msg[i];
	}
    if(HAL_CAN_AddTxMessage(&CAN2_Handler, &Tx2Header, message, &TxMailbox) != HAL_OK)//发送
	{
		return 1;
	}
